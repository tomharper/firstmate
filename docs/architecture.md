...
A task with an armed merge poll is exempt from that bound as well, because no completed turn is owed by a worker with nothing left to do.
...
Every routing decision into that absorb re-reads the poll and, unless the same pane hash was already classified complete, the authoritative crew state as well, so a retired poll or a failed follow-up run falls straight back to the wedge ladder.
