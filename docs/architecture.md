...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, completed tasks awaiting the merge or decision authority that remain idle past that same bound without a captain-relevant status line, and heartbeat backstop hits.
...
A task the watcher classifies complete is exempt from that bound as well, because no completed turn is owed by a worker with nothing left to do.
That exemption depends on the watcher having classified it, so it does not apply while away mode has the watcher one-shot; see the completion contract below.
That exemption is the same precedence-checked answer every other suppression uses, so a busy pane whose live state still reads `working`, `parked`, `blocked` or `failed` keeps the bound and escalates.
...
A crew whose work is complete and whose status log ends on a non-captain-relevant line shares that cadence for the same reason: its next move belongs to whoever holds merge or decision authority, so an idle pane is the expected outcome rather than a symptom.
A complete crew whose log already ends captain-relevant is absorbed once and never re-surfaced at all, because that line reached the captain through the signal path when it was written and repeating it would add a wake to the commonest healthy shape.
...
Every decision to suppress an alarm goes through one owner, `may_suppress_alarm` in `bin/fm-watch.sh`, which is the only place in that file allowed to read the poll.
It re-reads the poll every time and, unless the same pane hash was already classified complete, the authoritative crew state as well, so a retired poll or a failed follow-up run falls straight back to the wedge ladder.
`bin/fm-watch-suppression-check.sh` fails when a raw poll read appears anywhere else in that file, because three separate raw reads had already bypassed the precedence and silenced alarms the rule meant to keep.
...
Suppression requires evidence, and that rule is unconditional: a complete task is absorbed because supervision knows it is finished and waiting on a human, and that knowledge comes from the authoritative crew-state read.
Where the watcher does not pay that read, it does not know, and the task alarms.
Away mode is the case where that bites, so it is stated plainly rather than left to be discovered: while `state/.afk` exists the daemon owns triage and the watcher is one-shot, so it skips the read, and a completed task awaiting the captain's merge whose pane is busy and past `FM_BUSY_TURN_MAX_SECS` climbs the full wedge ladder for as long as away mode lasts.
That is the right outcome rather than a limitation.
An honestly documented limit beats a silently broken guarantee, the alarm is the safe direction under this change's governing rule, and the daemon batches away-mode escalations into one digest rather than waking per event, so being right here costs little.
The alternatives were weakening away mode's own no-costly-read contract, or suppressing on the durable record alone without the classifier, which is the precedence bypass this change has repeatedly had to close.
...
The always-on watcher also uses that library's absorb classification on no-verb signals, on first-sighting stale panes before status-log terminality is trusted, and at the alarm-suppression points above, bounded there to one read per pane hash, while the daemon maintains distinct wedge and declared-pause recheck cadences.
