...
# fm-completeness-check.sh is run (not sourced) by fm-teardown.sh through
# "$FM_ROOT/bin/", and exit 0 is its only proceed signal - an absent wrapper
# yields 127, which teardown treats as a gate that could not run and therefore
# blocks. It is copied with the engine and rules it resolves next to itself, so
# the synthetic bin/ reaches the same verdict a real one would.
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-nm-run-lib.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh fm-public-followup-lib.sh fm-secondmate-registry-lib.sh fm-x-lib.sh fm-completeness-check.sh fm-completeness.py fm-completeness.rules.json"
