...
To turn it on, install the optional dependency declared in [requirements.txt](requirements.txt) - `python3 -m pip install -r requirements.txt` - for the `python3` already on your `PATH`, since the gate imports the solver with that ambient interpreter.
