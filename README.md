...
- **Provable lifecycle claims** - an optional Z3-backed completeness gate proves each done, teardown, or merge claim consistent with the directives before the irreversible step, naming the invariant a blocked claim violated; without it the gate steps aside and the existing bash safety checks remain the hard guarantee.
To turn it on, make `z3` importable by the `python3` already on your `PATH`, however you prefer to provide it, since the gate imports it with that ambient interpreter and a virtualenv therefore counts only while that `python3` is the virtualenv's.
One example that satisfies it is `python3 -m pip install --break-system-packages z3-solver`, where the flag is needed because Homebrew and Debian mark their python externally-managed under PEP 668 and a bare `pip install` fails there; overriding that protection is a reasonable choice on a personal machine but is not the only way to meet the requirement.
- **Grounded crewmates** - per-repo ground truth kept in `data/repos/<key>.md` is injected into every scaffolded ship and scout brief, so workers start from a repo's real architecture and studied tool choices instead of re-improvising them.
