...
printf '%s\n' "$SNAPSHOT" | jq -r '
  ...
  def task_row($t):
    "| \($t.id) | \($t.current_state.state) / \($t.current_state.source) / \(dash($t.current_state.age)) | \($t.kind) | \(dash($t.backlog.repo // $t.project)) | \($t.backend) | \(endpoint_of($t)) | \(artifact($t)) | \(path_of($t)) | \(action_of($t)) |";
  def blocker($r):
